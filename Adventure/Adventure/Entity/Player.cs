...
namespace Adventure
{
    public class Player : Entity
    {
        ...
        public override void Update()
        {
            ...
            if (CanMove)
            {
                ...
                if (Velocity.X > 0 && Velocity.Y == 0)
                {
                    ...
                    pushVel1 = new Vector2(0, WALK_SPEED);
                    pushVel2 = new Vector2(0, -WALK_SPEED);
                    needToCheck = true;
                }
                ...
                else if (Velocity.X == 0 && Velocity.Y > 0)
                {
                    ...
                    expectedPosition3 = new Vector2(Position.X + Width - 1, Position.Y + Height + 1);
                    pushVel1 = new Vector2(WALK_SPEED, 0);
                    pushVel2 = new Vector2(-WALK_SPEED, 0);
                    needToCheck = true;
                }
                else if (Velocity.X == 0 && Velocity.Y < 0)
                {
            ...
            {
                //Keys[] arrowKeys = new Keys[] { Keys.Left, Keys.Right, Keys.Up, Keys.Down };
                //Keys[] pressedArrowKeys = gamepadState.GetPressedKeys().Intersect(arrowKeys).ToArray();
                //Keys[] previousPressedArrowKeys = previousGamepadState.GetPressedKeys().Intersect(arrowKeys).ToArray();
        ...
        public override void OnEntityCollision(Entity other)
        {
            ...
            if (other is KeyDoor)
            {
                if (AllignedWith(other) && inventory.NumKeys > 0)
                {
                    inventory.UseKey();
