...
namespace Adventure
{
    class Arrow : Entity
    {
        ...
        public override void Update()
        {
            ...
            if (area.GetCollisionAtCell(Area.ConvertPositionToCell(TipPosition)) == TileCollision.Impassable)
            {
                Rectangle cellRectangle = Area.CreateRectangleForCell(Area.ConvertPositionToCell(TipPosition));

                float intersectDistance = 0f;
                if (FaceDirection == Directions.Up)
                    intersectDistance = cellRectangle.Bottom - TipPosition.Y;
                else if (FaceDirection == Directions.Down)
                    intersectDistance = TipPosition.Y - cellRectangle.Top;
                else if (FaceDirection == Directions.Left)
                    intersectDistance = cellRectangle.Right - TipPosition.X;
                else if (FaceDirection == Directions.Right)
                    intersectDistance = TipPosition.X - cellRectangle.Left;
                ...
                if (intersectDistance >= 10)
                {
                    ...
                    if (FaceDirection == Directions.Up)
                        Position.Y = cellRectangle.Bottom - 10;
                    else if (FaceDirection == Directions.Down)
                        Position.Y = cellRectangle.Top + 10 - Height;
                    else if (FaceDirection == Directions.Left)
                        Position.X = cellRectangle.Right - 10;
                    else if (FaceDirection == Directions.Right)
                        Position.X = cellRectangle.Left + 10 - Width;
